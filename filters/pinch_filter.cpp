...
bool PinchFilter::Apply(const std::vector<double>& parameters) {
    ...
    double pinch_radius = parameters[0] * 100;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            if (distance < pinch_radius) {
                double r = distance / pinch_radius;
                double new_distance = std::sqrt(r) * pinch_radius;
                ...
                double r = pinch_radius / distance;
                double new_distance = std::sqrt(r) * distance;
                int new_x = static_cast<int>(center_x + new_distance * std::cos(theta));
                int new_y = static_cast<int>(center_y + new_distance * std::sin(theta));
                if (new_x >= 0 && new_x < width && new_y >= 0 && new_y < height) {
                    result[y][x] = image[new_y][new_x];
